import { farsiNumber, notify, jitterPreventedVersion, copyToClipboard, getTextContent, setTokenizedText, soon, getCursorLineAndPos, urlQueries2Json } from "./lib/common";
...
for (var selector in SELECTOR_TO_COMPONENT_MAP) {
  [].forEach.call(document.querySelectorAll(selector), (e) => SELECTOR_TO_COMPONENT_MAP[selector].applyTo(e));
}
...
class TargomanWebUiApp {
  constructor() {
    ...
    this.srcTypeDropdownDiv = document.querySelector('div.dropdown[data-bind-to="srcType"]');
    ...
    if (document.cookie.replace(/(?:(?:^|.*;\s*)hideMobileOverlay\s*\=\s*([^;]*).*$)|^.*$/, "$1") !== "true") document.querySelector("div#mobile_overlay").classList.add("visible");

    document.querySelector("div#mobile_overlay a.hide-on-click").addEventListener("click", (e) => {
      document.querySelector("div#mobile_overlay").classList.remove("visible");
      let now = new Date();
    ...
    if (window.location.hash.length > 1) {
      if (window.location.hash.startsWith("#!/g/")) BindHandler.setItemValue("srcText", decodeURIComponent(window.location.hash.substr("#!/g/en/".length)));
      else BindHandler.setItemValue("srcText", decodeURIComponent(window.location.hash.substr(1)));
    }

    window.onhashchange = () => BindHandler.setItemValue("srcText", decodeURIComponent(window.location.hash.substr(1)));
  }
  ...
  voteTranslation(up) {
    CommunityAPI.addAnonymousVote("ssid", BindHandler.srcText, getTextContent(this.tgtContentDiv).trim(), up ? "G" : "B")
      .then((r) => {
  ...
  acceptNewTranslation() {
    CommunityAPI.saveSuggestion("ssid", this.translationDirection, BindHandler.srcText, getTextContent(this.tgtContentDiv).trim())
      .then((r) => {
  ...
  handleHoverOverPhrases(e) {
    let clearPhraseHighlight = jitterPreventedVersion(() => [].forEach.call(document.querySelectorAll("div#content div.content span.phrase-token"), (e) => e.classList.remove("hi")));

    let highlightPhrase = jitterPreventedVersion((parIndex, phraseIndex) =>
      [].forEach.call(document.querySelectorAll(`div#content div.content span[data-par-index="${parIndex}"][data-phrase-index="${phraseIndex}"]`), (e) => e.classList.add("hi"))
    );

    if (e.type.toLowerCase() === "mouseenter") {
      if (e.target.nodeName === "SPAN" && e.target.classList.contains("phrase-token")) highlightPhrase(e.target.dataset.parIndex, e.target.dataset.phraseIndex);
  ...
  setCallbacks() {
    ...
    [].forEach.call(document.querySelectorAll(".engine-desc,.engine-info"), (e) => {
      e.addEventListener("transitionend", (e) => {
        if (e.target.style.opacity == 0) e.target.style.display = "";
      });
    });
    document.addEventListener(
      "click",
      (e) => {
        if (e.target === this.metadataDiv.querySelector("span.engine")) {
          let desc = document.querySelector(`.engine-desc.${e.target.dataset.class}`);
          if (desc) {
        ...
        if (e.target === this.metadataDiv.querySelector("span.translation-time")) {
  ...
  registerChangeHandlers() {
    ...
    BindHandler.registerChangeHandler("srcType", this.handleSourceTypeChange);
    BindHandler.registerChangeHandler("tgtLang", this.handleTargetLangChange);
    BindHandler.registerChangeHandler("proposingNewTranslationMode", (value) => {
      if (value) {
  ...
  handleSourceTextChange(value) {
    if (BindHandler.srcLang.updateDetectedLanguage) BindHandler.srcLang.updateDetectedLanguage(value);
    ...
    history.pushState({}, "ترگمان - ترجمه آنلاین و رایگان فارسی به انگلیسی و ترجمه انگلیسی به فارسی", `/`);
  }
      ...
      translationResult.map((parData) => {
        ...
        for (var e of parts) {
          let start = e[0][0],
            ...
            phraseIndex = e[2];
          if (start > sourceIndex) result.push([source.substring(sourceIndex, start)]);
          result.push([source.substring(start, end), phraseIndex]);
          sourceIndex = end;
        }
        if (sourceIndex < source.length) result.push([source.substring(sourceIndex)]);
        return result;
      })
    );
  }
    ...
    let tokenizationResult = translationResult.map((parData) => {
      ...
      for (var e of parts) {
        let start = e[1][0],
          ...
          phraseIndex = e[2];
        if (start > targetIndex) result.push([target.substring(targetIndex, start)]);
  ...
  updateDicResults(text, dicResult) {
    ...
    [].forEach.call(this.dicResults.querySelectorAll(".src"), (e) => (e.textContent = dicResult.word));
    ...
    fillPartItems(this.dicResultsRelExps, dicResult.relExp, dicResult.dir, (e, items) => {
      ...
      for (var word in dicResult.relExp) {
        e.appendChild(createLi("source", wordLink(word)));
        const ul = document.createElement("ul");
        for (let i = 0; i < dicResult.relExp[word].length; ++i) ul.appendChild(createLi("translation", wordLink(dicResult.relExp[word][i])));
        e.appendChild(ul);
        e.appendChild(createLi("separator"));
      }
    });
  ...
  updateEngineInfo(class_, time, timePerWord) {
    ...
    span.textContent = class_ in ENGINE_CLASS_TITLE ? ENGINE_CLASS_TITLE[class_] : "رسمی";
    ...
    this.metadataDiv.querySelector("span.translation-time").textContent = totalTimeStr;
    this.engineInfoDiv.querySelector("span.total-time").textContent = totalTimeStr;
    this.engineInfoDiv.querySelector("span.time-per-word").textContent = timePerWordStr;
    this.engineInfoDiv.querySelector("span.time-per-500words").textContent = timePer500WordsStr;
    this.metadataDiv.style.display = "block";
  }

  minifyBoxes(state) {
    [].forEach.call(document.querySelectorAll("div.content"), (el) => {
      if (state) el.classList.add("one-line");
      else el.classList.remove("one-line");
    });
  }
  ...
  translate() {
    ...
    allPromises.push(
      Translation.translate(sourceText, this.translationDirection, this.translationClass)
        .then((r) => {
          if (r === false || BindHandler.srcText != sourceText) return;
          this.updateTargetContentWithTokenizedText(r.tr);
          this.updateSourceContentWithTokenizedText(r.tr);
          this.updateEngineInfo(r.class, r.time, r.time / sourceText.split(/\S/).length);
        })
        .catch((e) => {
