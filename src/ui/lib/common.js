function isLineBreakingNode(n) {
  ...
  if (n.nodeName !== "BR" && getComputedStyle(n).display !== "block") return false;
  if (n.nodeName === "BR" && n.parentElement.lastChild === n) return false;
  let range = document.createRange();
  range.setStart(n.parentElement, 0);
  range.setEnd(n, 0);
  let previousText = range.toString();
  if (n.nodeName !== "BR" && previousText === "") return false;
...
export function getTextContent(e) {
  let result = "";
  let walker = document.createTreeWalker(e, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, (n) => {
    if (n.nodeType === Node.TEXT_NODE) result += n.textContent;
    else if (isLineBreakingNode(n)) result += "\n";
...
export function setTokenizedText(e, tokenizationResult) {
  ...
  for (let i = 0; i < tokenizationResult.length; ++i) {
    ...
    for (var token of parTokens) {
      ...
      } else newChild = document.createTextNode(token[0]);
...
export function getCursorLineAndPos(e) {
  ...
  if (!selection.focusNode || !e.contains(selection.focusNode)) return { cursorLine, cursorPos };
  ...
  if (focusNode === e) {
    if (focusOffset < e.childNodes.length) {
      ...
      focusOffset = focusNode.nodeType === Node.TEXT_NODE ? focusNode.textContent.length : focusNode.childNodes.length;
    }
  }
  cursorLine = 0;
  cursorPos = 0;
  let walker = document.createTreeWalker(e, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, (n) => {
    if (n.nodeType === Node.TEXT_NODE) {
      ...
      } else cursorPos += n.textContent.length;
    } else if (isLineBreakingNode(n)) {
      ++cursorLine;
      cursorPos = 0;
      if (focusNode === n) {
        for (let i = 0; i < focusOffset; ++i) cursorPos += n.childNodes[i].textContent.length;
        return NodeFilter.FILTER_ACCEPT;
      }
    } else if (focusNode === n) {
      for (let i = 0; i < focusOffset; ++i) cursorPos += n.childNodes[i].textContent.length;
...
export function setCursorLineAndPos(e, { cursorLine, cursorPos }) {
  ...
  let walker = document.createTreeWalker(e, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, (n) => {
    if (cursorLine > nodeLine) {
      if (isLineBreakingNode(n)) ++nodeLine;
    } else {
      if (n.nodeType === Node.TEXT_NODE) {
        focusNode = n;
...
export const notify = (function () {
  ...
  function notify(title, message, type) {
    ...
    hideDialogTimeout = setTimeout(() => dialog.classList.remove("shown"), 2000);
  }
